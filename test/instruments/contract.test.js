...
for (const spec of ALL) {
  ...
  test(`${spec.id}: scoring is pure and deterministic`, () => {
  ...
  test(`${spec.id}: a result survives a JSON round trip`, () => {
    ...
    assert.doesNotThrow(() => str(spec.view(revived, {})));
    assert.doesNotThrow(() => spec.instructions(revived));
  });

  test(`${spec.id}: renders without leaking objects into the markup`, () => {
    const html = str(spec.view(spec.score(answersFor(spec)), {}));
  ...
  test(`${spec.id}: instruction cards are usable and correctly channelled`, () => {
    for (let seed = 1; seed <= 8; seed++) {
      const cards = spec.instructions(spec.score(answersFor(spec, seed)));
  ...
  if (spec.compare) {
    test(`${spec.id}: compares two results without blowing up`, () => {
      ...
      const html = str(spec.compare(a, b, { nameA: "Ada", nameB: "Bo" }));
...
test("an empty answer set still scores rather than throwing", () => {
  for (const spec of registry.byFamily("questionnaire")) {
    assert.doesNotThrow(() => str(spec.view(spec.score({}), {})), spec.id);
  }
});
...
test("a hostile name is escaped everywhere it is rendered", () => {
  ...
  for (const html of [str(spec.view(result, {})), str(spec.compare(result, result, { nameA: evil, nameB: evil }))]) {
