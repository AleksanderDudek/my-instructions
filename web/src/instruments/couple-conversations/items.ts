 ...
 */
import type { Item, ItemTier, T } from "@/core/types";

/** Four states, because "we mentioned it once" is not the same as deciding. */
const STATUS = ["never", "passing", "talked", "decided"] as const;
export type Status = (typeof STATUS)[number];

/** The coarse three-state summary each topic reduces to. Never a score. */
const LEANS = ["a", "middle", "b"] as const;
export type Lean = (typeof LEANS)[number];

export type TopicId = "roles" | "money" | "conflict" | "children" | "religion";
export type Topic = { id: TopicId; positions: string[]; facts?: boolean };
...
const CHILD_FACTS: Record<string, string[]> = {
...
/**
 * Every item carries its topic so the runner can page by it and `score()` can
 * group without a second table. `topic` and `scaleName` ride alongside the
 * contract's own fields rather than replacing any of them.
 */
export type ConversationItem = Item & { topic: TopicId; scaleName?: string };

/** The form, built per topic. */
function itemsFor(t: T): ConversationItem[] {
    ...
    for (const id of topic.positions) {
      if (topic.facts) {
        ...
        items.push({
          ...
          topic: topic.id,
          prompt: t(`item.${id}`),
          ...(PRIVATE.has(id) ? { tier: "private" as ItemTier } : {}),
        });
      }
    }
