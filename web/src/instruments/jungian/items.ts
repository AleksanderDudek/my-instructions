 ...
 * for the reason written into love-languages/items.ts and enneagram/items.ts:
...
/**
 * The eight functions, as ids. Two jobs (perceiving, judging) times two kinds
 * (intuition/sensing, thinking/feeling) times two attitudes (outward, inward).
 */
export type FunctionKey = "ne" | "ni" | "se" | "si" | "te" | "ti" | "fe" | "fi";

/** The letter a function contributes to the four-letter code. */
export type CodeLetter = "N" | "S" | "T" | "F";

/* Structure only. Every word lives in i18n/, keyed by function id. */
const GLYPHS: Record<FunctionKey, string> = {
  ne: "◇", ni: "◆",
  se: "○", si: "●",
  te: "△", ti: "▲",
  fe: "♡", fi: "♥",
...
const PERCEIVING: FunctionKey[] = ["ne", "ni", "se", "si"];
const JUDGING: FunctionKey[] = ["te", "ti", "fe", "fi"];
const ORDER: FunctionKey[] = ["ne", "ni", "se", "si", "te", "ti", "fe", "fi"];

/** Every function's opposite: same job, other attitude and other pole. */
const OPPOSITE: Record<FunctionKey, FunctionKey> = { ne: "si", si: "ne", ni: "se", se: "ni", te: "fi", fi: "te", ti: "fe", fe: "ti" };

/** Outward-facing functions. The rest run inward. */
const EXTRAVERTED: ReadonlySet<FunctionKey> = new Set<FunctionKey>(["ne", "se", "te", "fe"]);

/** Which letter of the code each function contributes as a perceiver or judge. */
const LETTER: Record<FunctionKey, CodeLetter> = { ne: "N", ni: "N", se: "S", si: "S", te: "T", ti: "T", fe: "F", fi: "F" };

/** kind/scaleName are constant across this bank, so the rows stay readable. */
const row = (id: string, scale: FunctionKey, reverse = false) =>
